...
def connect_pixels(prev_pixel2_uv, current_pixel1_uv):
    """
    The indices returned are (previous, current)
    """
    indexes = []
    for i in range (len(current_pixel1_uv)):
        ...
        if mask.any():
            ...
            # print("Connecting pixel ", uv1, " at index ", i, " with previous pixel at index ", idx)
            # print("Previous pixel: ", prev_pixel2_uv[idx])
            # print("Current pixel: ", current_pixel1_uv[i])
...
def test_gtsam_pose_to_point():
    ...
    for index in range(1, 28):
        ...
        graph.add(gtsam.PriorFactorPose3(
            pose_0_key, P0, gtsam.noiseModel.Diagonal.Sigmas(sigmas)
        ))
        graph.add(gtsam.PriorFactorPose3(
        ...
        for i in range(len(current_obs_landmark_c1)):
            ...
            obs_landmark_c1_i = current_obs_landmark_c1[i]
            obs_landmark_c2_i = current_obs_landmark_c2[i]
            covar_scale = 1.
            obs_cov_landmark_c1_i = np.array(current_obs_cov_landmark_c1[i])*covar_scale
            obs_cov_landmark_c2_i = np.array(current_obs_cov_landmark_c2[i])*covar_scale
            ...
            # Window frame: add connected landmarks from previous pairs
            if any(i==connected_index[1] for connected_index in connected_indexes):
                previous_index = [connected_index[0] for connected_index in connected_indexes if connected_index[1]==i][0]
                obs_landmark_c0_i = prev_obs_landmark_c1[previous_index]
                obs_cov_landmark_c0_i = np.array(prev_obs_cov_landmark_c1[previous_index])*covar_scale
                noise_model_0 = gtsam.noiseModel.Gaussian.Covariance(obs_cov_landmark_c0_i)
                noise_model_0 = gtsam.noiseModel.Robust.Create(
                    m_huber,
                    noise_model_0
                )
                factor0 = make_pose_to_point_factor(pose_0_key, landmark_key, obs_landmark_c0_i, noise_model_0)
                graph.add(factor0)
        ...
        rerun_viz(current_from_idx, current_frame_idx,
